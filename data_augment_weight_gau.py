...
def init_dict(n):
    dict_origin = {}
    for i in range(n):
        upper_bound = (i + 1) * 2 / n -1
        dict_origin[upper_bound] = {'num': 0, 'max': (i + 1) * 2 / n -1, 'min': i * 2 / n -1, 'x': [], 'y': []}
        if i == 0:
            dict_origin[upper_bound]['min'] = -1.1
    return dict_origin
...
def divide(x_axis_data, y_axis_data, dict_new):
    ...
    while i != len(x_axis_data):
        for region in dict_new:
            if y_axis_data[i] > 1 or y_axis_data[i] < -1:
...
def gaussian_sample(t, dict_all, sample_num, t_max):
    min_len = 15
    labels_before = torch.Tensor([dict_all[i]['label'] for i in t])
    mean_before = torch.mean(labels_before)
    ...
    labels_after = torch.Tensor([dict_all[i]['label'] for i in t])
    std = torch.std(labels_after)
    if std == 0:
        prob = torch.ones_like(labels_after).to(device)
    else:
        prob = torch.Tensor(scipy.stats.norm.pdf(labels_after, mean_before, std)).to(device)

    weight = prob / torch.sum(prob)
    mean = torch.matmul(weight, sec_features).cpu().detach().numpy()
    cov = torch.cov(sec_features.T, aweights=weight).cpu().detach().numpy()
    ...
    for i in range(sample_num):
        ...
        sample_feature = torch.tensor(np.random.multivariate_normal(mean, cov), dtype=torch.float32).to(device)
        # sample_feature = torch.tensor(np.random.multivariate_normal(mean, cov), dtype=torch.float32)
...
def data_augment(features, target, movies, dict_sta):
    # n = 100
    # dict_new = init_dict(n)
    # time = [i for i in range(len(features))]
    # divide(time, target, dict_new)
    # smooth(dict_new)
    # with open(f'./dict_new/{movies}.pkl', 'wb') as f:
    #     pickle.dump(dict_new, f)
    #     f.close()
    # dict_new = pickle.load(open(f'./dict_new/31-10-100/{movies}.pkl', 'rb'))
    features_aug, target_aug = gaussian_aug(features, target, dict_sta)
