...
from data_augment_smogn import smogn_aug
from data_augment_cmixup import mixup_aug
import sys
...
class PGL_SUM(nn.Module):
    def __init__(self, input_size=256, output_size=128, freq=10000, pos_enc=None,
        ...
        self.cfg = Config()
        # self.dict_new = pickle.load(open('./dict_new/dict_new_list31.pkl', 'rb'))
        # 特征降维
    ...
    def forward(self, frame_features, target, aug, epoch, movies, dict_sta):
        ...
        if self.training and aug and epoch%10==0:
            y, aug_target = data_augment(y, target, movies, dict_sta)
            # y, aug_target = smogn_aug(y, target, dict_sta)
            # y, aug_target = mixup_aug(y, target, dict_sta)
        else:
            aug_target = target
            # dict_new = self.dict_new
