...
class PGL_SUM(nn.Module):
    def __init__(self, input_size=1024, output_size=1024, freq=10000, pos_enc='absolute',
                 num_segments=4, heads=8, fusion='add'):
        ...
        self.linear_f = nn.Linear(in_features=3584, out_features=1024)
    ...
    def forward(self, frame_features, target, aug, epoch):
        ...
        :return: A tuple of:
            ...
            attn_weights: Tensor with shape [T, T] containing the attention weights.
        """
        vggish_features, bgm_emotion_features, emotion_features, scene_features, pose_features = frame_features
        frame_features = torch.cat((vggish_features, bgm_emotion_features, emotion_features, scene_features, pose_features), dim=1)
        frame_features = self.linear_f(frame_features)
        ...
        if self.training and aug and epoch%10==0:
            y, aug_target = data_augment(y, target)
        else:
            aug_target = target
